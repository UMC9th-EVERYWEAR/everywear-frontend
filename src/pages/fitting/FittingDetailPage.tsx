...
// 	const {
// 		data: fittingDetail,
// 		isLoading: isFittingDetailLoading,
// 	} = useFittingDetail(fittingId);
...
// 	/* ------------------------------
//     상세 응답에서 productId 추출
//   ------------------------------ */
// 	const realProductId = useMemo(() => {
// 		if (!fittingDetail) return null;

// 		return (
// 			(fittingDetail as any).productId ??
//       (fittingDetail as any).product_id ??
//       (fittingDetail as any).product?.productId ??
//       null
// 		);
// 	}, [fittingDetail]);
...
// 		return (
// 			products.find(
// 				(p) => Number(p.product_id) === Number(realProductId),
// 			) ?? null
// 		);
// 	}, [isSuccess, products, realProductId]);

// 	/* ------------------------------
//     기존 컴포넌트에 맞게 normalize
//   ------------------------------ */
// 	const normalizedProduct = useMemo<ListDTO | null>(() => {
// 		if (!detailProduct) return null;

// 		const raw = detailProduct as unknown as Record<string, string | number>;
...
// 		const detail = fittingDetail as any;

// 		const historyResultUrl =
//       detail?.fittingResultImage ||
//       detail?.fittingResultImageUrl;
...
// 		if (isFittingSuccess) {
// 			const res = resultFitting as any;

// 			return {
// 				status: 'success',
// 				resultUrl:
//           res?.fittingResultImageUrl ||
//           res?.fittingResultImage ||
//           '',
// 			};
// 		}

// 		return { status: 'idle' };
// 	}, [fittingDetail, isFittingLoading, isFittingSuccess, resultFitting]);

// 	const handleHeart = (status: boolean) => {
// 		if (!normalizedProduct?.product_id) return;
...
// 	if (isFittingDetailLoading || isProductsLoading) {
// 		return (
