...
const ReviewCard = ({ data }: ReviewCardProps) => {
	...
	return (
		<div className="flex flex-col gap-1 py-1.5 border-b border-solid border-neutral-100 dark:border-neutral-800 w-full overflow-hidden">
			...
			<div className="py-3.25 pl-3.25 bg-neutral-50 dark:bg-neutral-800 border rounded-xs border-black/10 dark:border-neutral-700 gap-0.75 flex flex-col text-regular-14 text-neutral-600 dark:text-neutral-400">
				<span>구매 정보 : {data.option_text ?? ''}</span>
				{data.user_height && data.user_weight && 
					<span>체형 정보 : {data.user_height}cm, {data.user_weight}kg</span>
				}
			</div>

			<div className="relative">
				<p className="text-regular-14 text-neutral-900 dark:text-neutral-100 break-all tracking-[-0.42px]">
					{displayContent}
					{shouldTruncate && !isExpanded && (
						<button
							onClick={() => setIsExpanded(true)}
							className="text-regular-14 text-neutral-400 dark:text-neutral-500 inline-block cursor-pointer ml-1"
						>
							더보기
						</button>
					)}
